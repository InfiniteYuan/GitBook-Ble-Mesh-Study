...
> 最初启动网状设备电源时，不会对其进行配置。这意味着它没有节点地址，也没有任何其他配置信息。在这种状态下，通电后，它将发出“未配置”的信标，供应商将根据该信标进行操作。要成为网状网络的一部分，必须首先配置该设备。**为此，mesh 网络供应商会将配置信息发送到未配置的设备。**
...
> 例如，在一个拥有 50 个灯和 20 个交换机的办公室中，**只有供应商知道所有 70 个设备的节点地址。**各个灯和开关不需要此信息。供应者为每个办公区域创建组地址，然后根据适用的组地址在交换机中设置发布地址，在灯光中设置订阅地址。当电灯开关的状态更改时，它会发布一个组地址，并且适用的订阅电灯也会相应地工作。
...
> **Mesh 网络运行不需要 provisioner。**仅在必须从网络中**添加或删除节点**（将其列入黑名单）时才需要使用此功能。 因此，不存在可能导致整个网络瘫痪的中心故障点。
...
> **到消息广播时，它们已被加密两次。**一次使用应用程序密钥，第二次使用网络密钥。**每个密钥的长度为 128 位，并且加密算法在 CCM 模式下使用 AES。**由于它不了解任何密钥，因此不会加密应用程序层上的传出消息。另外，应用程序不知道或不在乎其自己的节点地址。当应用程序发送消息时，将添加目标地址，并由基础网格堆栈执行加密。 关键信息（例如节点地址，应用程序密钥，网络密钥，发布地址，订阅列表，密钥绑定和其他配置信息）完全由 smartBASIC 固件管理。仅在供应者将设备配置到网络中之后，此信息才可用。
